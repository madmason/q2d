...
void Logger::log(QString message, shared_ptr<LogLevel> severity)
{
...
void Logger::log(QStringList messages, shared_ptr<LogLevel> severity)
{
...
void Logger::log(QString message, QString severity)
{
    this->log(message, m_manager->logLevel(severity));
}

void Logger::log(QStringList messages, QString severity)
{
    this->log(messages, m_manager->logLevel(severity));
}

QList<std::shared_ptr<LogEntry>> Logger::entries() const
{
    return m_entries;
}
